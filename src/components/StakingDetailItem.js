...
export default function StakingDetailItem({ data }) {
    const stakeItem = async () => {
        ...
        try {
            const req = {
                ...
                tx_id: data.tx_id,
                value: data.value,
                sat_ordinal: data.sat_ordinal,
                sat_rarity: data.sat_rarity,
                sat_coinbase_height: data.sat_coinbase_height,
                content_type: data.content_type,
                timestamp: data.timestamp,
            }
            const response = await fetch("http://192.168.15.75:3000/stack", {
                method: "POST",
                headers: {
    ...
    return (
        <div className="justify-center items-center p-4">
            ...
            <div className="justify-end items-end">
                <Button className="" onClick={stakeItem}>Stack</Button>
            </div>
        </div>
    );
}
